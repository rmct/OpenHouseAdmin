...
public class CommandManager implements CommandExecutor
{
	...
	protected class CommandHandler
	{
		...
		public CommandHandler(Object handler, Method method, PluginCommand command)
		{
			...
			OpenHouseCommand cAnnotation = method.getAnnotation(OpenHouseCommand.class);
		...
		public boolean execute(CommandSender sender, World world, String[] args)
			throws CommandPermissionException, UnrecognizedOptionException
		{
			...
			OpenHouseCommand command = method.getAnnotation(OpenHouseCommand.class);
			OpenHousePermission permissions = method.getAnnotation(OpenHousePermission.class);
			...
			try { return ((Boolean) method.invoke(handler, sender, world, args, cli)).booleanValue(); }
	...
	{
		OpenHouseAdmin plugin = OpenHouseAdmin.getInstance();
		...
		try
		{
			...
			return handler == null ? false : handler.execute(sender, world, args);
		}
